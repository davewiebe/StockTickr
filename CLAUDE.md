...
- Phases: `lobby` → `countdown` (3s) → `playing` (15s frozen pre-roll, then dice tick) → `ended`. Host can `paused` ⇄ `playing` mid-game (freezes dice + trades + end-clock, preserving remaining ms in `pauseRemainingMs`).
...
- 6 stocks: GOLD SILV OIL BOND INDU GRAIN. 18-face action die: per magnitude (5/10/20) = 3 up, 2 down, 1 dividend. No split/bankrupt die face. Stock hitting $0 → shares wiped, resets to par $100 (`event.bankrupt`). Stock reaching $200 → shares doubled, resets to par $100 (`event.split`).
- Socket events: `room:create|join|leave|start|updateSettings|pause|resume`, `trade:buy|sell`, server→client `game:started|countdown|open|preroll|rolling|tick|ended|callout|trade|paused|resumed`, `room:player*|settingsUpdated`.
